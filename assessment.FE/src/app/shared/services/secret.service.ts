...
export class SecretService {
  ...
  getSubscriptions(): Observable<ISubscriptions[]> {
    return this.http.get(this.baseUrl + 'api/subscription/'+localStorage.getItem('id'),
    this.getHttpOptions())
      .pipe(map((res: any) => res));
  }

  Subscribe(bookId: any): Observable<ISubscriptions> {
    return this.http.post(this.baseUrl + 'api/subscription', { userId: localStorage.getItem('id'), bookId: bookId},
  ...
  Unsubscribe(bookId: any): Observable<ISubscriptions> {
    return this.http.post(this.baseUrl + 'api/subscription/unsubscribe', { userId: localStorage.getItem('id'), bookId: bookId},
    this.getHttpOptions()).pipe(
      map((response: any) => {
        return response;
      })
